...
Created on Thurs February 15 22:57:01 2018
...
#                                  (analogous to ggplot2)
import numpy as np  # import Numpy
import os  # imports OS module - used for saving/reading files
os.chdir('C:/Users/ckell/Desktop/Research/Python/introduction_to_python')
...
# Save your plot with the plt.savefig function, to working directory
plt.plot(xData, yData)  # plots the x-y data on the current figure
fname = 'output\\MyPlotImage.png'  # choose filename & extension
#                                   (e.g. png, jpg, pdf)
...
# this is equivalent to clicking on the file
...
#                   It's gone forever. ** Like the rm Linux function.
...
myDir = os.getcwd()  # example folder to save figure
filepath = myDir + "\\" + fname  # combine directory & file name into 1 string
print(filepath)  # where the image will be saved
plt.savefig(filepath, dpi=320)  # Save file, dpi is dots per inch (resolution)
...
# colon specifies the line style
...
#  This will plot two plots, one after another
plt.figure()  # make new figure
plt.plot(xData, yData, 'rx')  # red X markers
plt.xlabel("Velocity [mph]")  # add x-label to plot
plt.ylabel("Stopping Distance [ft]")  # add y-label to plot
plt.figure()  # make another new figure
plt.plot(xData2, yData2, 'bs')  # blue square markers
plt.xlabel("Velocity [mph]")  # add x-label to plot
plt.ylabel("Stopping Distance [ft]")  # add y-label to plot
...
###
# Simple OLS regression for (xData, yData)
###

degree = 1  # Choose order of polynomial
# create fit object with inputs (x, y, degree)
fit = np.polyfit(xData, yData, degree)  # y = b_0 + b_1*x + b_2*x^2
yFit = np.polyval(fit, xData)  # Calculate y_fit for each x data point
corr_coeff = np.corrcoef(xData, yData)[0, 1]  # Get correlation coefficient (R)
print(corr_coeff)  # Print correlation coefficient
Rsq = corr_coeff**2  # Calculate R^2 value
print('The R-squared value is {0:0.6}'.format(Rsq))  # Print R^2 value

# Yfit is the fitted values
plt.plot(xData, yData, 'ko', xData, yFit, 'r')  # Plot data with regression
# default is a blue line, so by default it does a line, but now it is red
plt.xlabel("Velocity [mph]")  # add x-label to plot
plt.ylabel("Stopping Distance [ft]")  # add y-label to plot
...
a = 3.2
type(a)  # this is a float, or a 32-bit float
